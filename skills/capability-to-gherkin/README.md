...
│   └── capability-to-gherkin/
│       ├── SKILL.md               # Canonical skill definition (source of truth)
│       ├── README.md              # This file
│       ├── evals/
│       │   └── evals.json         # Eval scenarios
│       ├── references/
│       │   ├── gherkin-reference.md    # Gherkin syntax guide + framework step-def conventions
│       │   ├── openspec-integration.md  # OpenSpec workflow guide
│       │   ├── examples/
│       │   │   └── customer-management-example.md
│       │   └── condensed.md      # Fallback for tools that don't read SKILL.md
│       ├── scripts/
│       │   └── converter.py      # Optional automation utility
│       └── assets/
│           ├── feature-template.feature  # Gherkin Feature template
│           └── scenario-examples.md      # Scenario generation examples
...
Skills are discovered by tools at well-known paths. `install-skill.sh` symlinks this skill
folder into a target project under the path each tool expects:
...
tool's rules file. The condensed version carries the trigger phrases, the conversion pipeline,
mapping rules, and the checklist without the verbose rationale.
